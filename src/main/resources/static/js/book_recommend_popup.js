export function BookForm(lists) {
    ...
    let list = [];
    console.log(lists)
    Object.keys(lists).forEach(bookTitle => {
        lists[bookTitle].forEach(bookInfo => {
            const author = bookInfo.author;
            const genres = bookInfo.genre;
            const image = bookInfo.image;
            const info = bookInfo.info;
            const title = bookInfo.title;

            list.push([title, image, author, genres, info]);
        });
    });
    console.log(list)
    for (let i = 0; i < list.length; i++) {
        ...
        img2.setAttribute('src', `${list[i][1]}`)
